...
import { ChevronLeft, Phone, Truck, Sparkles } from "lucide-react";
...
export default async function ProductDetailPage({
  ...
  const waText = encodeURIComponent(
    `Hi! I'm interested in the "${product.name}" arrangement.`
  );
        ...
        <div className="flex flex-col">
          ...
          <ul className="mt-10 space-y-3 border-t border-border/60 pt-6 text-sm text-muted-foreground">
            <li className="flex items-center gap-3">
              <Sparkles className="h-4 w-4 text-primary" /> Hand-tied with fresh
              seasonal stems
            </li>
            <li className="flex items-center gap-3">
              <Truck className="h-4 w-4 text-primary" /> Same-day local delivery
              available
            </li>
          </ul>
        </div>
